...
public class AttendanceViewModel extends AndroidViewModel {
    ...
    private MutableLiveData<Boolean> isAttendanceExisted;
    private MutableLiveData<Boolean> isCreateStressful;
    ...
    public AttendanceViewModel(@NonNull Application application) {
        ...
        isAttendanceExisted = new MutableLiveData<>();
        isCreateStressful = new MutableLiveData<>();
        //get data
    ...
    public MutableLiveData<Boolean> getIsAttendanceExisted() {
        return isAttendanceExisted;
    }

    public MutableLiveData<Boolean> getIsCreateStressful() {
        return isCreateStressful;
    }
    ...
    public void CreateAttendance(Attendance attendance) {
        db.collection("Attendance")
                ...
                .addOnSuccessListener(documentReference -> {
                    ...
                    isCreateStressful.setValue(true);
                })
                .addOnFailureListener(e -> {
                    Log.d(TAG, "Error adding document", e);
                    isCreateStressful.setValue(false);
                });
    }

    public void CheckExistedAttendance(String time) {
        Log.d("xxx", "CheckExistedAttendance: " + time);
        db.collection("Attendance")
                .whereEqualTo("attendance_ID", time)
                .get()
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            if (task.getResult().isEmpty() == true) {
                                isAttendanceExisted.setValue(false);
                            } else {
                                isAttendanceExisted.setValue(true);
                            }
                        } else {
                            Log.d(TAG, "Error getting documents: ", task.getException());
                        }
    ...
    public void ResetListStudentAttendance() {
